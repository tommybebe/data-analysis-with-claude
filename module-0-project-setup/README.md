...
- 로컬 개발 환경(uv, dbt, Claude Code CLI)을 정상 설치하고 검증할 수 있습니다 (marimo는 이후 모듈에서 도입)
...
├── pyproject.toml              ← Python 의존성 (dbt-bigquery, 데이터 생성 라이브러리 — marimo는 이후 모듈)
...
# (marimo는 이후 모듈에서 도입합니다 — 모듈 0에서는 설치·검증하지 않습니다)
...
**성공 기준**: `uv`, `dbt`, `claude` 세 명령 모두 오류 없이 버전 번호 출력 (marimo는 이후 모듈에서 도입)
...
models:
  fittrack:
    # staging과 marts는 profiles.yml의 dataset(analytics)에 그대로 생성됩니다.
    # +schema를 지정하면 dbt-bigquery가 <dataset>_<schema> 형태로 데이터셋 이름을
    # 이어붙여(예: analytics_analytics) 예상과 다른 데이터셋이 생기므로 사용하지 않습니다.
    staging:
      +materialized: view
    marts:
      +materialized: table
...
> **인증 방식 (로컬 개발 vs CI)**: 로컬 개발에서는 `method: oauth`(gcloud ADC)를 권장합니다.
> `gcloud auth application-default login`을 한 번 실행하면 서비스 계정 키파일 없이 dbt가
> BigQuery에 연결되며, `GOOGLE_APPLICATION_CREDENTIALS`가 필요 없습니다. 서비스 계정
> 키파일(`method: service-account`)은 모듈 4의 CI(GitHub Actions) 경로에서 사용합니다.
> 아래 템플릿은 두 타깃(`dev`=oauth 로컬, `ci`=service-account)을 모두 포함합니다.

```bash
# 홈 디렉터리에 dbt 설정 디렉터리 생성
mkdir -p ~/.dbt

# profiles.yml 작성 (로컬 개발: oauth / CI: service-account)
cat > ~/.dbt/profiles.yml << 'EOF'
fittrack:
  target: dev
  outputs:
    # 로컬 개발용 (권장): gcloud ADC 기반 oauth — 키파일 불필요
    dev:
      type: bigquery
      method: oauth
      ...
      timeout_seconds: 300
      threads: 4
    # CI/자동화용 (모듈 4): 서비스 계정 키파일 (--target ci)
    ci:
      type: bigquery
...
#
# 로컬 개발: dev 타깃(method: oauth) — gcloud ADC 사용, 키파일 불필요.
#   먼저 `gcloud auth application-default login` 으로 ADC를 설정하세요.
# CI(모듈 4): ci 타깃(method: service-account) — GOOGLE_APPLICATION_CREDENTIALS 키파일 사용.
#   dbt 실행 시 `--target ci` 로 선택합니다.
fittrack:
  target: dev
  outputs:
    # 로컬 개발용 (권장): gcloud ADC 기반 oauth — 키파일 불필요
...
2. `~/.dbt/profiles.yml`의 `project` 값과 인증 설정 확인 (oauth: `gcloud auth application-default login` 실행 여부 / service-account: `keyfile` 경로)
3. 인증 주체(oauth는 로그인 계정, service-account는 서비스 계정)에 `bigquery.dataEditor` + `bigquery.jobUser` 권한 확인
4. 환경 변수 `GCP_PROJECT_ID` 설정 확인 (service-account 방식은 `GOOGLE_APPLICATION_CREDENTIALS`도 확인)
...
| # | 항목 | 검증 방법 | 성공 기준 |
|---|------|-----------|-----------|
| 0 | 환경 변수 설정 | `.env` 파일 기반 환경 변수 확인 | `GCP_PROJECT_ID` 설정 + 서비스 계정 키파일(`GOOGLE_APPLICATION_CREDENTIALS`) 또는 gcloud ADC(oauth) 인증 |
...
| 5 | marimo (선택) | `uv run marimo --version` | 선택 항목 — marimo는 이후 모듈에서 도입 (모듈 0에서는 미설치여도 통과) |
